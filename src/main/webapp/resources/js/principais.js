/* @refatorar
 * Já existe uma função que retorna o ip, analisar a mesma e adaptar a função abaixo
 * */
function obterIpPorta(){
	...
	var temporaria = fIp.indexOf(":");
	
	var fPosicaoInicioPorta  =  temporaria + 1;
	
	var fPosicaoFimPorta = fPosicaoFim;
	
	var ip = fIp.substring(fPosicaoInicioPorta, fPosicaoFimPorta);
...
function princ_concatenarValorComp(vComp, vValor){ 
	
	if(vComp == null||vValor == null){}else{
		
		if($("#"+vComp)[0].tagName == "TEXTAREA"){
			
			var fValorAtual = String($("#"+vComp).val());
			
			if(fValorAtual.length > 0){
			
				$("#"+vComp).jqteVal(fValorAtual +"<br>"+ vValor);	
...
function princ_execAjaxJqueryRet1(vUrl, vComp){
...
function princ_alterarClass(vComp, vClassAtual, vClasseNova){
	
	$("#"+vComp).removeClass(vClassAtual).addClass(vClasseNova);
	
}

function princ_FullScreen() {
...
function princ_antesdeinserir(vValor){
	
	return vValor.replace(" – ","aaa").replace(/'/g,"''''").replace(/&nbsp;/,"").replace(/"/g,"''''");
	
}


function princ_limparForm(vForm){
	document.getElementById(vForm).reset();
}

function princ_existe(vValor, vVerifica){	
	if(vValor.indexOf(vVerifica) > 0 ){
		return true;
	}else{
		return false;
	}	
...
function princ_obterBrowser(){	
	if( princ_existe(navigator.userAgent,"Chrome/" ) ){
		return "Chrome";
	}else if( princ_existe(navigator.userAgent,"Firefox/" ) ){
		return "Firefox";
	};
}
